    ...
    for (;;) {
        ...
        switch (gs->state) {
            case GS_HEADER:
                ...
                if (!ssl2gs ||
                    ssl3_isLikelyV3Hello((const unsigned char*)&gs->hdr)) {
                    ...
                    /* Probably an SSLv2 record header. */
                ...
                /* This is the max fragment length for an encrypted SSLv3+
                 * fragment plus the size of the record header. */
                if (!v2HdrLength &&
                    gs->remainder > (MAX_FRAGMENT_LENGTH + 2048 + 5)) {
...
ssl3_GatherCompleteHandshake(sslSocket *ss, int flags)
{
    ...
    ssl2Gather ssl2gs;
    int rv;
    PRBool keepGoing = PR_TRUE, acceptV2Hello;
    ...
    acceptV2Hello = ss->sec.isServer && ss->ssl3.hs.ws == wait_client_hello;
