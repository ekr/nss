...
static SECStatus tls13_ChangeCipherSpec(sslSocket *ss, TrafficKeyType type,
...
static SECStatus tls13_HandleNewSessionTicket(sslSocket *ss, SSL3Opaque *b,
                                              PRUint32 length);
static void
tls13_CombineHashes(sslSocket *ss, PRUint8 *hhash, unsigned int hlen,
                    TLS13CombinedHash *hashes);
                                      ...
                                      PRUint32 length, SSL3Hashes *hashesPtr)
{
    ...
    switch (ss->ssl3.hs.msg_type) {
        ...
        case finished:
            ...
            tls13_CombineHashes(ss, hashesPtr->u.raw, hashesPtr->len,
                                &hashes);
            return tls13_HandleFinished(ss, b, length, &hashes);
...
tls13_ComputeEarlySecrets(sslSocket *ss, PRBool setup0Rtt)
{
    ...
    /* Extract off the resumptionPsk (if present), else zeroes */
...
tls13_ComputeHandshakeSecrets(sslSocket *ss)
{
    ...
    rv = tls13_HkdfExpandLabel(ss->ssl3.hs.hsTrafficSecret,
                               tls13_GetHash(ss),
                               NULL, 0,
                               ...
                               &ss->ssl3.hs.clientFinishedSecret);
    if (rv != SECSuccess) {
                             ...
                             sslSessionID *sid)
{
    ...
    rv = tls13_ComputeEarlySecrets(ss, do0Rtt);
    if (rv != SECSuccess) {
        FATAL_ERROR(ss, SEC_ERROR_LIBRARY_FAILURE, internal_error);
        goto loser;
    }

    /* Set ourselves up to read the early handshake keys.
     * We need to still be writing with the null keys. */
    if (do0Rtt) {
    ...
    if (!do0Rtt) {
        ...
        rv = tls13_ChangeCipherSpec(ss, TrafficKeyEarlyHandshake,
                                    InstallCipherSpecRead);
        if (rv != SECSuccess) {
            FATAL_ERROR(ss, PORT_GetError(), handshake_failure);
            goto loser;
...
tls13_SendServerHelloSequence(sslSocket *ss)
{
    ...
    rv = tls13_ChangeCipherSpec(ss, TrafficKeyHandshake,
                                InstallCipherSpecWrite);
    if (rv != SECSuccess) {
        FATAL_ERROR(ss, PORT_GetError(), internal_error);
        return SECFailure;
    ...
    rv = tls13_ChangeCipherSpec(ss, TrafficKeyApplicationData,
                                InstallCipherSpecWrite);
    if (rv != SECSuccess) {
        FATAL_ERROR(ss, SEC_ERROR_LIBRARY_FAILURE, internal_error);
        return SECFailure;
    ...
    rv = tls13_ChangeCipherSpec(ss, do0Rtt ?
                                TrafficKeyEarlyApplicationData :
                                TrafficKeyHandshake,
                                InstallCipherSpecRead);
    if (rv != SECSuccess) {
...
tls13_HandleServerHelloPart2(sslSocket *ss)
{
    ...
    /* Call in here so we can check the properties. */
    rv = ssl3_FillInCipherSuite(ss);
    if (rv != SECSuccess) {
        FATAL_ERROR(ss, SEC_ERROR_LIBRARY_FAILURE, internal_error);
        return SECFailure;
    }
...
tls13_DeriveTrafficKeys(sslSocket *ss, ssl3CipherSpec *spec,
                        TrafficKeyType type,
                        InstallCipherSpecDirection direction)
{
...
#define EXPAND_TRAFFIC_IV(purpose_, target_)                    \
    ...
    switch (type) {
        case TrafficKeyEarlyHandshake:
            ...
            prk = ss->ssl3.hs.earlyTrafficSecret;
            break;
        ...
        case TrafficKeyHandshake:
            ...
            prk = ss->ssl3.hs.hsTrafficSecret;
            break;
    ...
    if ((direction == InstallCipherSpecWrite) ^ (ss->sec.isServer)) {
...
static SECStatus
tls13_ChangeCipherSpec(sslSocket *ss, TrafficKeyType type,
                       InstallCipherSpecDirection direction)
{
    SECStatus rv;
    ssl3CipherSpec *spec = NULL;
    ssl3CipherSpec **specp = (direction == InstallCipherSpecRead) ?
            &ss->ssl3.crSpec : &ss->ssl3.cwSpec;
    ...
    spec->refCt = 1; /* One for read and one for write. */
    PR_APPEND_LINK(&spec->link, &ss->ssl3.hs.cipherSpecs);
                             ...
                             TLS13CombinedHash *hashes)
{
    ...
    if (ss->ssl3.hs.hashType == handshake_hash_unknown) {
         ...
         * This happens when we are doing 0-RTT. */
...
tls13_SendEncryptedExtensions(sslSocket *ss)
{
    ...
    // TODO(ekr@rtfm.com): There won't be any senders here if we are
    // a client.
...
tls13_MaybeDo0RTTHandshake(sslSocket *ss)
{
    ...
    rv = tls13_ChangeCipherSpec(ss, TrafficKeyEarlyApplicationData,
                                InstallCipherSpecWrite);
    if (rv != SECSuccess) {
        return rv;
    }
