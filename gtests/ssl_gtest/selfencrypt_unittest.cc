...
class SelfEncryptTestBase : public ::testing::Test {
 public:
  ...
  void SetUp() {
    ...
    ImportKey(&aes1_, kAesKey1, slot_.get(), CKM_AES_CBC);
    ImportKey(&aes2_, kAesKey2, slot_.get(), CKM_AES_CBC);
    ImportKey(&hmac1_, kHmacKey1, slot_.get(), CKM_SHA256_HMAC);
  ...
  void SelfTest(
    ...
    if (!error_code) {
      ...
      EXPECT_EQ(error_code, PORT_GetError());
    }
  }
...
// Use a different key which randomizes the plaintext. Because
// we have a full padding block this produces a padding error
// with very high probability.
TEST_F(SelfEncryptTest128, BadPadding) {
...
TEST_F(SelfEncryptTest128, MacWithAESKey) {
